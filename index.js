...
function processUsageDataGeneral(usage)
{
    let down
    let up
    if (config.IP != null) {
        let ifaceName
        for(let i=0;i<usage.interfaces.length;++i)
        {
        ...
        for(let i=0;i<usage.rxtx.length;++i)
        {
...
async function initNetworkCheck(configuration) {
    ...
    try {
        ...
        else if (config.testType == 'speedtest' && config.linuxDistro == 'ubuntu') {
            let browser = null
            try {
        ...
        else if (config.testType == 'speedtest' && config.linuxDistro == 'alpine') {
